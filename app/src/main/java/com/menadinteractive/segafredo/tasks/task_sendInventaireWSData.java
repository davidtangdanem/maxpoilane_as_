...
public class task_sendInventaireWSData extends AsyncTask<Void, Void, Integer>{
	...
	public task_sendInventaireWSData(Context context,Handler h){
		super();
	...
	//		if (inv.isInventaireTermine()==false) return 0;
			...
			if (ident.contains("test"))
			{
				...
				String queryHdr="SELECT * FROM "+
						...
						Global.dbKDLinInv.FIELD_LIGNEINV_TYPEPIECE+" <>'' ) order by "+
						Global.dbKDLinInv.FIELD_LIGNEINV_TYPEPIECE+" desc, "+
						Global.dbKDLinInv.FIELD_LIGNEINV_DATE;
				...
				if (resultWS.equals(""))
				{
					Global.dbKDLinInv.Reset( "SYNCHRO OK")	;
